...
DECLARE
    new_transaction_id integer = (SELECT id + 1 FROM transactions ORDER BY id DESC LIMIT 1 );
BEGIN
   ...
   -- SAVEPOINT just_created_transaction_row;
    ...
    IF balance_sum - money_number < account_limit THEN
        ...
        ROLLBACK;
        INSERT INTO transactions VALUES (new_transaction_id,clock_timestamp(),source_account,destination_account,money_number,'rollback');
       -- UPDATE transactions
        --SET status = 'rollback'
